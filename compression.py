...
def generate_all_transformed_blocks(img, block_size, step):
    transformed_blocks = []
...
def make_codes(img, block_size):
    img_wd = img.shape[0]
    img_ht = img.shape[1]
    adj_wd = img_wd - img_wd % block_size
    adj_ht = img_ht - img_ht % block_size
    for x in range(adj_wd, block_size):
        for y in range(adj_ht, block_size):
    ...
    for i in range(i_count):
        ...
        for j in range(j_count):
            ...
            min_d = float('inf')
def compress(img, block_size, step):
    img_wd = img.shape[0]
    img_ht = img.shape[1]
    transformations = []
    transformed_blocks = generate_all_transformed_blocks(img, block_size, step)
    i_count = img_wd // destination_size
    j_count = img_ht // destination_size
    for i in range(i_count):
        transformations.append([])
        for j in range(j_count):
            print("{}/{} ; {}/{}".format(i, i_count, j, j_count))
